...
public abstract class Object
{
    ...
    /// <summary>GameObject 专属销毁/实例化逻辑挂接点（Scene 层静态构造注册；引擎内部装配委托，非反射）。</summary>
    public static Action<Object>? GameObjectDestroyHook;
    public static Func<Object, Object>? GameObjectInstantiateHook;

    internal bool _destroyPending;
    internal bool _destroyed;

    /// <summary>是否已请求销毁（Destroy 标记后、帧末提交前为 true）。</summary>
    public bool IsDestroyPending => _destroyPending;

    /// <summary>是否已完成销毁提交（帧末提交后为 true）。</summary>
    public bool IsDestroyed => _destroyed;

    /// <summary>帧末销毁提交专用：写入已销毁标志（SceneManager/快照提交路径调用；幂等）。</summary>
    public void MarkDestroyed() => _destroyed = true;

    /// <summary>销毁标记写入入口（Scene 层递归销毁路径使用；幂等）。</summary>
    public void MarkDestroyPending() => _destroyPending = true;
