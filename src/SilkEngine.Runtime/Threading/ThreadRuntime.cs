...
public sealed class ThreadRuntime : IDisposable, IThreadGuard
{
    ...
    public ThreadDomain CurrentDomain => ReferenceEquals(Thread.CurrentThread, _mainThread)
        ? ThreadDomain.Main
        : _ambient.Value;
    ...
    public IDisposable EnterRender() => Enter(ThreadDomain.Render);
    ...
    public void RegisterManagedLoop(IManagedLoop loop)
    {
        if (IsDisposed)
    ...
    public void Drain(MainThreadPhase phase) => ((MainThreadDispatcher)MainThread).Drain(phase);
