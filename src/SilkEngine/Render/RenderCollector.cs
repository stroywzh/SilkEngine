...
{
    /// <summary>本批可渲染对象列表</summary>
    public IReadOnlyList<IRenderable> Renderers { get; init; } = [];
}
...
public sealed class RenderCollector
{
    ...
    /// <param name="renderables">已过滤的可渲染对象列表（仅含 Enabled 且层级活跃项）</param>
    /// <param name="camera">选中的相机视图（列表首个；列表为空时为 null）</param>
    /// <param name="batches">组装出的渲染批次列表（无渲染器时为空）</param>
    public void Gather(
    ...
    )
    {
        ...
        batches = renderables.Count > 0
            ? [new RenderBatch { Renderers = renderables }]
            : [];
    }
}
